      ...
      return DefaultTabController(
        ...
        child: Scaffold(
          ...
          bottomNavigationBar: BottomNavigationBar(
            ...
            selectedLabelStyle:
                TextStyle(color: Color(0xFF003F62), fontSize: 15),
            unselectedLabelStyle: TextStyle(fontSize: 13),
            selectedItemColor: Color(0xFF003F62),
            items: List.generate(widget.menus.length, (index) {
              final item = widget.menus[index];
              return BottomNavigationBarItem(
                icon: item.icon != null
                    ? Icon(item.icon, color: Color(0xFF3A5A98))
                    : SvgPicture.asset(
                        item.imageAsset!,
                        color: selectedIndex == index
                            ? Color(0xFF003F62)
                            : Colors.grey,
                        width: selectedIndex == index ? 25 : 20,
                        height: selectedIndex == index ? 25 : 20,
                      ),
                label: item.label,
              );
            }),
